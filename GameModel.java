...
public class GameModel implements Cloneable {
   ...
   /**
    ...
    private GenericLinkedStack<GameModel> uStack;
    private GenericLinkedStack<GameModel> rStack;

    public GameModel(int size) {
        generator = new Random();
        uStack = new GenericLinkedStack<GameModel>();
        rStack = new GenericLinkedStack<GameModel>();
        sizeOfGame = size;
        reset();
    }
   ...
   /**
    ...
    public String toString(){
     ...
     public GameModel clone(){
        ...
        try{
            ...
            DotInfo[][] clonedModel = new DotInfo[sizeOfGame][sizeOfGame];
        
            for(int i = 0 ; i < sizeOfGame; i++) {
                for(int j = 0; j < sizeOfGame; j++) {
                    clonedModel[i][j] = model[i][j];
                    if(model[i][j].isCaptured()) {
                        clonedModel[i][j].setCaptured(true);
                    }
                }
            }
            cloned.model = clonedModel;
            return cloned;
    ...
    /*public void undo() {
        rStack.push((GameModel) this.clone());
        restoreGameModel(uStack.pop());
    }
    
    public void redo(){
        uStack.push((GameModel) this.clone());
        restoreGameModel(rStack.pop());
    }
    
     private void restoreGameModel(GameModel a){
        currentSelectedColor = a.currentSelectedColor;
        numberOfSteps = a.numberOfSteps;
        model=a.model;
    }

    public static void main(String [] args){
        GameModel test = new GameModel(10);
        GameModel x;
        x = test.clone();
        test.capture(0,0);
        ...
        for(int i = 0; i < test.getSize(); i++) {
            for(int j = 0; j < test.getSize(); j++) {
                System.out.println(isCaptured(i,j));
            }
        }
        ...
        for(int i = 0; i < test.getSize(); i++) {
            for(int j = 0; j < test.getSize(); j++) {
                System.out.println(x.isCaptured(i,j));
            }
        }
    }*/
}
